...
function worldPointFromPointer(event: Pick<PointerEvent | MouseEvent, "clientX" | "clientY">, planeZ = 0) {
  if (!camera || !renderer) {
    return null;
  }
...
function rayPayloadFromPointer(event: Pick<PointerEvent | MouseEvent, "clientX" | "clientY">) {
  if (!camera || !renderer) {
    return null;
  }
  const rect = renderer.domElement.getBoundingClientRect();
...
function handlePointerMove(event: PointerEvent) {
  ...
  const point = worldPointFromPointer(event, interactionStartPoint.z);
  if (!point) {
    return;
  }
