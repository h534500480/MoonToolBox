...
ALL_TOOL_DEFINITIONS = [
    ...
    ToolDefinition(
        ...
        title="全局重定位候选点",
        subtitle="候选点生成与审核",
        description="用于搭建 Scan Context 全局重定位离线候选点生成工具页，当前先提供点云预览、候选点查看、框选删除、回收站和人工补点框架。",
        primary_action="确认候选点",
        secondary_action="选择",
        fields=[
            ToolField(key="input_pcd", label="输入 PCD", placeholder="G:/path/map.pcd"),
            ToolField(key="candidate_file", label="候选点文件", placeholder="G:/path/global_relocalization_db/candidates.csv"),
            ToolField(key="output_dir", label="输出目录", value="G:/ros_proj/ros_tool/output_global_relocalization"),
